...
const MOCK_ITEMS: GridItem[] = [
  {
    ...
    type: "rate",
    title: "Пацаны",
    description: "Четвертый сезон просто разрыв всего, Хоумлендер выдает исторический перформанс, смотреть всем обязательно",
  ...
  },
  {
    ...
    description: "Жду полноценный стек дизайна iOS 26 с полным стеклянным неоморфизмом. Текущие виджеты Apple — это просто прошлый век, интерфейсы должны дышать и быть абсолютно чистыми от визуального мусора. Кто согласен, накидайте трендов в ленту",
    commentsCount: 24,
    viewsCount: "3.4k",
    repostsCount: 12,
  },
  ...
  },
  {
    ...
    description: "Ребята, если запускаете свои проекты, сразу думайте про клиентские фильтры картинок. Наплыв неадекватов, желающих загрузить непотребства в общую ленту, начнется в первую же минуту после релиза",
    commentsCount: 8,
    viewsCount: "1.1k",
    repostsCount: 3,
  },
    ...
    <div className="grid grid-cols-2 gap-3.5 px-5 w-full auto-rows-max pb-24">
      {MOCK_ITEMS.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className={`flex flex-col bg-neutral-50 dark:bg-neutral-800/60 rounded-[28px] overflow-hidden p-4 relative cursor-pointer transition-colors duration-150 ${
            ...
            {isTake ? (
              <>
                ...
                <div className="flex flex-col pr-14">
                  ...
                  <p className="text-[11px] font-medium text-neutral-400 dark:text-neutral-500 leading-relaxed mt-1 break-words">
                    {item.description}
                  </p>
                </div>
                ...
                >
                  <div className="flex items-center space-x-1.5">
                    <img src="/icons/comments.png" className="w-3.5 h-3.5 object-contain dark:brightness-0 dark:invert opacity-45" alt="" />
                    <span>{item.commentsCount}</span>
                  </div>
                  <div className="flex items-center space-x-1.5">
                    <img src="/icons/views.png" className="w-3.5 h-3.5 object-contain dark:brightness-0 dark:invert opacity-45" alt="" />
                    <span>{item.viewsCount}</span>
                  </div>
                  <div className="flex items-center space-x-1.5">
                    <img src="/icons/reposts.png" className="w-3.5 h-3.5 object-contain dark:brightness-0 dark:invert opacity-45" alt="" />
                    <span>{item.repostsCount}</span>
                  </div>
                </div>
              </>
            ) : (
              /* Карточка Оценки (rate) */
              <>
                {item.imageUrl && (
                  <div className="w-full rounded-[20px] overflow-hidden bg-neutral-100 dark:bg-neutral-800 relative aspect-[3/4]">
                    <img
                      src={item.imageUrl}
                      alt={item.title}
                      loading="lazy"
                ...
                <div className="flex flex-col pt-3 px-0.5 relative">
                  <h3 className="text-xs font-bold text-appleLight-text dark:text-appleDark-text truncate pr-1">
                    {item.title}
                  </h3>

                  <div className="relative mt-1 max-h-[34px] overflow-hidden">
                    <p className="text-[11px] font-medium text-neutral-400 dark:text-neutral-500 leading-tight break-words">
                      {item.description}
                    </p>
                    <div className="absolute bottom-0 left-0 right-0 h-3 bg-gradient-to-t from-neutral-50 dark:from-neutral-800/100 to-transparent pointer-events-none" />
                  </div>
                </div>
              </>
            )}
