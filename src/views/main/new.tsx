...
export default function NewModal({ isOpen, onClose }: NewModalProps) {
  ...
  // Сохраняем контент в плашках при перелистывании
  ...
  // Фиксация высоты от сжатия клавиатурой
  useEffect(() => {
    if (isOpen) {
  ...
  // Переключение таббаром — БЕЗ анимации, но с вибрацией
  const handleTabClick = (tabId: SubTabType) => {
  ...
  // Логика свайпов с защитой от вертикального скролла
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    // Если начался вертикальный скролл контента — полностью блокируем свайп табов
    if (!isHorizontalSwipe.current && Math.abs(deltaY) > Math.abs(deltaX) && Math.abs(deltaY) > 6) {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    // Переключение свайпом — С анимацией и вибрацией
  ...
  const physicsState = useRef({
    x: 0, tx: 0, vx: 0, w: 0, tw: 0, vw: 0,
    sx: 1, tsx: 1, vsx: 0, sy: 1, tsy: 1, vsy: 0,
    isMoving: false,
  });

  // Пружина капли таббара
  useEffect(() => {
    if (!isOpen) return;
  ...
  useEffect(() => {
    ...
    fetch(`/icons/${targetJson}.json`)
      ...
      .catch((err) => console.error("Ошибка Lottie:", err));
  }, [activeSubTab, isOpen]);
  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col justify-end transition-all duration-300 ${isOpen ? "pointer-events-auto" : "pointer-events-none"}`}>
      ...
      <div 
        style={{ height: isOpen ? modalHeight : "0px" }}
        className={`relative w-full bg-white dark:bg-neutral-900 rounded-t-[32px] shadow-2xl flex flex-col overflow-hidden transition-all duration-300 cubic-bezier(0.15, 1, 0.2, 1) will-change-transform ${isOpen ? "translate-y-0" : "translate-y-full"}`}
      >
        {/* Шапка */}
          ...
          <div className="w-full h-11 bg-appleLight-secondaryBg dark:bg-appleDark-secondaryBg p-1 box-border rounded-full flex items-center relative mb-5 select-none">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`flex-1 h-full rounded-full text-xs font-medium outline-none whitespace-nowrap flex items-center justify-center z-20 transition-colors duration-300 ${activeSubTab === tab.id ? "text-appleLight-text dark:text-appleDark-text" : "text-appleLight-text/45 dark:text-appleDark-text/45"}`}
              >
                {tab.label}
              </button>
            ))}
        ...
        {/* Лента контента со свайпом */}
        <div className="flex-1 w-full overflow-hidden" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
          <div 
            className="flex w-[400%] h-full will-change-transform"
            style={{ 
              transform: `translateX(-${activeIndex * 25}%)`,
              transition: animateTransition ? "transform 300ms cubic-bezier(0.15, 1, 0.2, 1)" : "none"
            }}
          >
            <div className="w-[25%] h-full flex flex-col overflow-hidden">
              <RateView title={rateTitle} setTitle={setRateTitle} description={rateDescription} setDescription={setRateDescription} animationData={activeSubTab === "rate" ? animationData : null} />
            </div>
            <div className="w-[25%] h-full flex flex-col overflow-hidden">
              <TakeView title={takeTitle} setTitle={setTakeTitle} description={takeDescription} setDescription={setTakeDescription} animationData={activeSubTab === "take" ? animationData : null} />
            </div>
            <div className="w-[25%] h-full flex flex-col items-center justify-center text-center select-none">
              <span className="text-xl mb-2">🛠️</span>
              <p className="text-xs font-bold text-neutral-400 dark:text-neutral-600 uppercase tracking-widest">В разработке</p>
            </div>
            <div className="w-[25%] h-full flex flex-col items-center justify-center text-center select-none">
              <span className="text-xl mb-2">🛠️</span>
              <p className="text-xs font-bold text-neutral-400 dark:text-neutral-600 uppercase tracking-widest">В разработке</p>
