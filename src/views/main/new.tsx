          ...
          <div className="w-full h-11 relative">
            ...
            >
              ...
              >
                ...
                {/* Кнопка поиска внутри табов (размер 36x36) */}
                <button
                  onClick={enableSearch}
                  className="w-9 h-9 ml-1.5 bg-white dark:bg-neutral-700 shadow-sm rounded-full flex items-center justify-center p-[20%] outline-none active:scale-95 transition-transform z-20 flex-shrink-0"
                >
                  <img 
              ...
              {/* Кнопка Плюс (размер 44x44) */}
              <button
                onClick={onAddClick}
                className={`h-11 bg-appleLight-secondaryBg dark:bg-appleDark-secondaryBg rounded-full flex items-center justify-center p-[20%] outline-none active:scale-95 transition-all duration-300 cubic-bezier(0.16, 1, 0.3, 1) will-change-transform flex-shrink-0 ${
                  shouldHideAdd 
            ...
            >
              ...
              {/* Кнопка Крестик в поиске (размер 32x32) */}
              <button 
                onClick={disableSearch}
                className="w-8 h-8 flex items-center justify-center rounded-full bg-appleLight-text/10 dark:bg-white/10 p-[20%] outline-none active:scale-90 transition-transform flex-shrink-0"
              >
                <img 
                  src="/icons/cross.png" 
                  alt="Очистить" 
                  className="w-full h-full object-contain dark:brightness-0 dark:invert opacity-60"
                />
              </button>
            </div>
